		...
		<table class="table table-hover table-condensed table-bordered">
			...
			if (isset($_SESSION['consulta'])) {
				if ($_SESSION['consulta'] > 0) {
					...
					$sql = "SELECT * FROM persona p, llamada c WHERE p.id_persona = c.id_persona and c.id_persona = '$idp' ORDER BY p.id_persona DESC";
				}else{
					$sql = "SELECT * FROM persona p, llamada c WHERE p.id_persona = c.id_persona ORDER BY p.id_persona DESC";
				}
			}else{
				$sql = "SELECT * FROM persona p, llamada c WHERE p.id_persona = c.id_persona ORDER BY p.id_persona DESC";
			}
			...
			while ($ver = mysqli_fetch_row($resultado)) {
				...
				<tr>
					...
					<td>
						<form action="../librerias/reportellamadas2.php" method="POST">
							<input type="hidden" id="id_persona" name="id_persona" value="<?php echo $ver[0];?>">
